...
export default function ProfileScreen() {
  ...
  const handleSignOut = async () => {
    Alert.alert('تسجيل الخروج', 'هل أنت متأكد؟', [
      ...
      { text: 'تسجيل الخروج', style: 'destructive', onPress: async () => { await signOut(); router.replace('/(tabs)/'); } },
    ]);
  };
  ...
  const menuItems = [
    ...
    { title: 'لوحة الصدارة', icon: BarChart, onPress: () => router.push('/leaderboard'), color: '#fbc531' },
    { title: 'عجلة الحظ', icon: Gift, onPress: () => router.push('/spin-wheel'), color: '#8B5CF6' },
